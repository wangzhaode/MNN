...
make -j8
cp $ANDROID_NDK/sources/cxx-stl/llvm-libc++/libs/armeabi-v7a/libc++_shared.so .
cp *.so $PACKAGE_PATH/armeabi-v7a
popd
...
make -j8
cp $ANDROID_NDK/sources/cxx-stl/llvm-libc++/libs/arm64-v8a/libc++_shared.so .
cp *.so $PACKAGE_PATH/arm64-v8a
popd
